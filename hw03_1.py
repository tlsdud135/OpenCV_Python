...
def onMouse(event, x, y, flags, param):
    ...
    if event == cv2.EVENT_MOUSEMOVE:
        if not isDragging:
            img_draw = img.copy()
            # 드래그 영역 표시
            pts = np.array([[x - half, y - half], [x + half, y - half], [x, y]], np.int32)
            ...
            img_draw = img.copy()
            pts = np.array([[cx1 - half, cy1 - half], [cx1 + half, cy1 - half], [x, y]], np.int32)
            cv2.polylines(img_draw, [pts], True, (0, 0, 255))
            pts = np.array([[cx1 - half, cy1 - half], [cx1 - half, cy1 + half], [x, y]], np.int32)
            cv2.polylines(img_draw, [pts], True, (0, 0, 255))
            pts = np.array([[cx1 - half, cy1 + half], [cx1 + half, cy1 + half], [x, y]], np.int32)
            cv2.polylines(img_draw, [pts], True, (0, 0, 255))
...
if __name__ == '__main__':
    img = cv2.imread("images/bono.jpg")
    h, w = img.shape[:2]
